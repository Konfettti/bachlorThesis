...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional directory to store per-split prediction CSV files. "
            "Useful when RelBench hides targets and you need artefacts for official submissions."
        ),
    )
...
def main() -> None:
    ...
    if args.save_predictions:
        prediction_dir = Path(args.save_predictions).expanduser().resolve()
        prediction_dir.mkdir(parents=True, exist_ok=True)
        print(f"Saving per-split predictions to '{prediction_dir}'.")
    ...
    if label_status:
        ...
        if any(status != "labels available" for status in label_status.values()):
            ...
            if prediction_dir:
                print(
                    "Saved prediction files can be submitted to the official RelBench evaluation service "
                    "to obtain metrics for hidden-label splits."
                )
    ...
    for split_name, (obs_df, y_series) in splits.items():
        ...
        if prediction_dir is not None:
            prediction_frame = obs_df[["observation_id"]].copy()
            if task.entity_col in obs_df.columns:
                prediction_frame[task.entity_col] = obs_df[task.entity_col]

            if task.task_type == TaskType.REGRESSION:
                prediction_frame["prediction"] = y_pred.astype(np.float32)
            else:
                if encoder is None:
                    raise AssertionError("Encoder must be available for classification predictions.")
                class_predictions = encoder.inverse_transform(y_pred.astype(int))
                prediction_frame["prediction"] = class_predictions
                if hasattr(model, "predict_proba"):
                    proba = model.predict_proba(X_split)
                    classes = [str(label) for label in encoder.classes_]
                    for idx, class_label in enumerate(classes):
                        prediction_frame[f"proba_{class_label}"] = proba[:, idx]

            saved_path = prediction_dir / f"{split_name}_predictions.csv"
            prediction_frame.to_csv(saved_path, index=False)
        ...
        if task.task_type == TaskType.REGRESSION:
            if y_series is None:
                ...
                if saved_path is not None:
                    print(f"[{split_name}] Predictions saved to '{saved_path}'.")
                continue
